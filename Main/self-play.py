...
def self_play(model_file_name = "3d-tic-tac-toe.zip", total_timesteps = 10000, play_count = 10, verbose=1):
    ...
    for count in range(play_count):
        ...
        env.reset()
        default_env.reset()
        env.set_opponent(ModelOpponent(model2, default_env))
        model1.learn(total_timesteps=total_timesteps)
        env.set_opponent(ModelOpponent(model1, default_env))
...
if __name__ == "__main__":
    self_play(total_timesteps=1000, play_count=2)